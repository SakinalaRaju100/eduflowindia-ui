...
export default function StudentCalendar() {
  ...
  const { data: diaryData } = useQuery({
    ...
    queryFn: () =>
      studentAPI.getDiaryNotes({ classroomId: fullData?.data?.data?.profile?.classroom?._id }),
  ...
  const leaveMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries(['student-leaves']);
      setLeaveOpen(false);
    },
  });
  ...
  const getDayExams = (day) =>
    exams.filter((e) => e.subjects?.some((s) => s.date && isSameDay(new Date(s.date), day)));
  const getDayLeave = (day) =>
    leaves.find(
      (l) => l.status === 'approved' && new Date(l.fromDate) <= day && new Date(l.toDate) >= day,
    );
  const getDayDiaryNotes = (day) =>
    diaryNotes.filter((n) => n.date && isSameDay(new Date(n.date), day));
    ...
    <Box>
      ...
      <Grid container spacing={2.5} sx={{ mb: 3 }}>
        {[
          ...
          ['Absent', totalDays - presentDays, '#E53935'],
        ...
        <CardContent sx={{ p: 2 }}>
          ...
          <Grid container>
            {Array.from({ length: firstDayOffset }).map((_, i) => (
              <Grid item xs={12 / 7} key={`e${i}`} sx={{ minHeight: 80 }} />
            ))}
            {days.map((day) => {
                ...
                <Grid item xs={12 / 7} key={day.toISOString()} sx={{ minHeight: 80, p: 0.3 }}>
                  <Box
                    ...
                    sx={{
                      ...
                      '&:hover': {
                        bgcolor: today ? 'primary.main' : 'action.selected',
            ...
            leaves.map((l) => (
              ...
              >
                <Box>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    {l.fromDate ? format(new Date(l.fromDate), 'dd MMM') : ''} –{' '}
                    {l.toDate ? format(new Date(l.toDate), 'dd MMM yyyy') : ''} ({l.totalDays} days)
                  </Typography>
                </Box>
                <Chip
                  label={l.status}
      ...
      >
        <DialogTitle fontWeight={700}>
          Details for{' '}
          {selectedDay ? format(new Date(selectedDay + 'T00:00:00'), 'dd MMMM yyyy') : ''}
        </DialogTitle>
        <DialogContent dividers sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            <Box>
              ...
              {selStatus ? (
                <Chip
                  ...
                  size="small"
                  sx={{
                    bgcolor: STATUS_COLORS[selStatus] || 'grey.500',
                    color: '#fff',
            ...
            {selLeave && (
              <Box>
                ...
                <Chip
                  label={`Approved: ${selLeave.leaveType} leave`}
                  color="success"
                  size="small"
                  sx={{ fontWeight: 600 }}
                />
            ...
            <Box>
              ...
              {selEvs.map((ev) => (
                <Box
                  key={ev._id}
                  sx={{
                    bgcolor: (ev.color || '#1565C0') + '15',
                    borderLeft: `3px solid ${ev.color || '#1565C0'}`,
                    p: 1.5,
                    borderRadius: 1,
                    mb: 1,
                  }}
                >
                  <Typography variant="body2" fontWeight={700} color={ev.color || '#1565C0'}>
                    {ev.title}
                  </Typography>
                  {ev.description && (
                    <Typography variant="caption" color="text.secondary">
          ...
          {selExs.length > 0 && (
            <Box>
              ...
              {selExs.map((ex) => (
                ...
                >
                  <Typography variant="body2" fontWeight={700} color="#FF6F00">
                    {ex.title}
                  </Typography>
                  <Typography variant="caption" color="text.secondary">
                    {ex.type} • {ex.term}
          ...
          {selNotes.length > 0 && (
            <Box>
              ...
              {selNotes.map((note) => (
                ...
                >
                  <Typography variant="body2" fontWeight={700} color="#00796B">
          ...
          {!selStatus &&
            ...
            selNotes.length === 0 && (
              <Typography variant="body2" color="text.secondary">
                No special details scheduled for this day.
              </Typography>
            )}
        </DialogContent>
        <DialogActions sx={{ p: 2 }}>
          <Button onClick={() => setDayDetailsOpen(false)} color="inherit">
            Close
          </Button>
          {selCanApplyLeave && (
            <Button
      ...
      <Dialog open={leaveOpen} onClose={() => setLeaveOpen(false)} maxWidth="xs" fullWidth>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            <FormControl fullWidth size="small">
              ...
              <Select
                ...
                label="Leave Type"
              >
                {['sick', 'casual', 'medical', 'personal', 'other'].map((t) => (
        ...
        <DialogActions sx={{ p: 2.5 }}>
          ...
          <Button
            variant="contained"
            onClick={() =>
              leaveMutation.mutate({ ...leaveForm, fromDate: selectedDay, toDate: selectedDay })
            }
