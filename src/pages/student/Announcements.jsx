...
export default function StudentAnnouncements() {
  ...
  const { data: annData } = useQuery({ queryKey: ['announcements'], queryFn: () => announcementAPI.getAll() });
  const announcements = annData?.data?.data || [];

  const readAnnMutation = useMutation({ mutationFn: id => announcementAPI.markRead(id), onSuccess: () => qc.invalidateQueries(['announcements']) });
        ...
        <CardContent sx={{ p: 3 }}>
          {filteredAnnouncements.length === 0
            ...
            : filteredAnnouncements.map((a, i) => (
                <Box key={a._id} onClick={() => readAnnMutation.mutate(a._id)} sx={{ cursor: 'pointer', mb: 2 }}>
                  {i > 0 && <Divider sx={{ mb: 2 }} />}
                  <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start' }}>
                    ...
                    <Box sx={{ flex: 1 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                        ...
                        <Typography variant="caption" color="text.secondary">{a.createdAt ? format(new Date(a.createdAt), 'dd MMM') : ''}</Typography>
                      </Box>
                      <Typography variant="body2" color="text.secondary" sx={{ mt: 0.3 }}>{a.content}</Typography>
                      <Typography variant="caption" color="text.secondary">— {a.createdBy?.firstName} {a.createdBy?.lastName} ({a.createdBy?.role})</Typography>
                    </Box>
                  </Box>
                </Box>
              ))
