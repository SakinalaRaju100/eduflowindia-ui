...
export default function TeacherDashboard() {
  ...
  const { data: anns } = useQuery({ queryKey:['announcements'], queryFn:()=>announcementAPI.getAll() });
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2.5} sx={{ mb:3 }}>
        <Grid onClick={()=>navigate('/teacher/classes')} item xs={6} sm={3}><StatCard title="My Classes" value={filteredClasses.length} icon={<Class/>} color="#1565C0" /></Grid>
        <Grid item xs={6} sm={3}><StatCard title="Pending Leaves" value={pendingLeaves.length} icon={<BeachAccess/>} color="#A26900" /></Grid>
        <Grid item xs={6} sm={3}><StatCard title="Student Leave reqquests" value={pendingLeaves.length} icon={<Assignment/>} color="#E65100" /></Grid>
        <Grid item xs={6} sm={3}><StatCard title="Announcements" value={filteredAnnouncements.length} icon={<Message/>} color="#6A1B9A" /></Grid>
      </Grid>
      <Grid container spacing={2.5}>
        <Grid item xs={12} md={6}>
          <Card elevation={0} sx={{ border:'1px solid', borderColor:'divider', borderRadius:3 }}>
            <CardContent sx={{ p:3 }}>
              <Typography variant="h6" fontWeight={700} gutterBottom>My Classes</Typography>
              {filteredClasses.map((c,i) => (
                <Box key={c._id}>
                  {i>0 && <Divider sx={{my:1}}/>}
                  <Box onClick={()=>navigate(`/teacher/classes/${c._id}`)} sx={{ display:'flex', justifyContent:'space-between', alignItems:'center', cursor:'pointer', p:1, mx:-1, borderRadius:1, '&:hover':{bgcolor:'action.hover'} }}>
                    <Box>
                      <Typography variant="body2" fontWeight={700}>{c.name}</Typography>
                      <Typography variant="caption" color="text.secondary">{c.students?.length||0} students · Room {c.roomNumber||'—'}</Typography>
                    </Box>
                    <Chip label={`Grade ${c.grade}`} size="small" color="primary" />
                  </Box>
                </Box>
              ))}
              {filteredClasses.length === 0 && <Typography color="text.secondary">No classes assigned for the selected year</Typography>}
            </CardContent>
          </Card>
        </Grid>
        <Grid item xs={12} md={6}>
              ...
              {filteredAnnouncements.slice(0,4).map((a,i) => (
                <Box key={a._id}>
                  ...
                  <Box sx={{ display:'flex', gap:1, alignItems:'flex-start' }}>
                    <Chip label={a.priority} size="small" color={a.priority==='urgent'?'error':a.priority==='important'?'warning':'default'} sx={{flexShrink:0}} />
                    <Box><Typography variant="body2" fontWeight={600}>{a.title}</Typography>
                      <Typography variant="caption" color="text.secondary">{a.content?.slice(0,80)}...</Typography></Box>
                  </Box>
                </Box>
              ))}
