...
export default function TeacherSalaries() {
  ...
  const cols = [
    { key: 'period', label: 'Month/Year', render: r => <Typography variant="body2" fontWeight={600}>{r.month} {r.year}</Typography> },
    { key: 'baseAndAllowances', label: 'Base + Allowances', render: r => `₹${(r.baseSalary + (r.allowances || 0)).toLocaleString()}` },
    { key: 'deductions', label: 'Deductions', render: r => `₹${(r.deductions || 0).toLocaleString()}` },
    { key: 'netSalary', label: 'Net Salary', render: r => <Typography variant="body2" color="primary" fontWeight={700}>₹{r.netSalary?.toLocaleString()}</Typography> },
    { key: 'status', label: 'Status', render: r => <Chip label={r.status} size="small" color={r.status === 'paid' ? 'success' : 'warning'} sx={{ textTransform: 'capitalize' }} /> },
    { key: 'actions', label: 'Actions', sortable: false, render: r => (
      <Tooltip title="Download Payslip"><IconButton size="small" color="primary" onClick={(e) => { e.stopPropagation(); handleDownload(r); }}><Download fontSize="small" /></IconButton></Tooltip>
    )},
  ];
  ...
  return (
    <Box>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h5" fontWeight={700}>My Salary Details</Typography>
      </Box>
