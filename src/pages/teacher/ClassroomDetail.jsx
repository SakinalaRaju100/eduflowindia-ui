...
export default function TeacherClassroomDetail() {
  ...
  const { data: diaryData } = useQuery({
    queryKey: ['diary', id],
    queryFn: () => studentAPI.getDiaryNotes({ classroomId: id }),
  });
  const diaryNotes = diaryData?.data?.data || [];

  const diaryMutation = useMutation({
    mutationFn: (d) =>
      editDiaryId ? studentAPI.updateDiaryNote(editDiaryId, d) : studentAPI.createDiaryNote(d),
    onSuccess: () => {
      qc.invalidateQueries(['diary', id]);
      setDiaryOpen(false);
      setDiaryForm({
        content: '',
        homework: '',
    ...
    <Box>
        ...
        <Grid item xs={12} lg={4}>
          <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 3 }}>
            <CardContent sx={{ p: 3 }}>
              ...
              >
                <Typography variant="h6" fontWeight={700} sx={{ fontSize: 12 }}>
                  Diary Notes
                </Typography>
                <Button
                  size="small"
                ...
                diaryNotes.map((n) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <Box
                        sx={{ display: 'flex', gap: 0.5, opacity: 0.8, '&:hover': { opacity: 1 } }}
                      >
                        <IconButton size="small" onClick={() => handleEditNote(n)}>
                          <Edit sx={{ fontSize: 18 }} />
                        </IconButton>
                        <IconButton
                          size="small"
                          color="error"
                          onClick={() => {
                            if (window.confirm('Delete this diary note?'))
                              delDiaryMutation.mutate(n._id);
                          }}
                        >
      ...
      <Dialog open={diaryOpen} onClose={() => setDiaryOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle fontWeight={700}>
          {editDiaryId ? 'Edit Diary Note' : 'Add Diary Note'}
        </DialogTitle>
        <DialogContent>
          {diaryError && (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            <TextField
              ...
              onChange={(e) => setDiaryForm((p) => ({ ...p, homework: e.target.value }))}
            />
          </Box>
        </DialogContent>
        <DialogActions sx={{ p: 2.5 }}>
          ...
          <Button
            variant="contained"
            onClick={() => diaryMutation.mutate({ ...diaryForm, classroom: id })}
