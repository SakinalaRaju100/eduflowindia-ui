...
export default function TeacherLeaves() {
  ...
  const myLeaves = leaves.filter(
    (l) => l.applicant?._id === user?._id || l.applicant === user?._id,
  );
  const pendingLeaves = leaves.filter(
    (l) => l.status === 'pending' && l.applicantRole === 'student',
  );

  const applyMutation = useMutation({
    mutationFn: (d) => leaveAPI.apply(d),
    onSuccess: () => {
      qc.invalidateQueries(['t-leaves']);
      setOpen(false);
      setForm(EMPTY);
      setError('');
    },
  ...
  const myCols = [
    {
      key: 'leaveType',
      label: 'Type',
      render: (r) => <Chip label={r.leaveType} size="small" variant="outlined" />,
    },
    {
      key: 'fromDate',
      label: 'From',
      render: (r) => (r.fromDate ? format(new Date(r.fromDate), 'dd MMM yyyy') : '—'),
    },
    {
      key: 'toDate',
      label: 'To',
      render: (r) => (r.toDate ? format(new Date(r.toDate), 'dd MMM yyyy') : '—'),
    },
    ...
    { key: 'reason', label: 'Reason', minWidth: 200 },
    {
      key: 'status',
      label: 'Status',
      render: (r) => <Chip label={r.status} size="small" color={STATUS_COLOR[r.status]} />,
    },
  ];
        ...
        r.status === 'pending' ? (
          <Box sx={{ display: 'flex', gap: 0.5 }}>
            <Button
              ...
              color="success"
              variant="outlined"
              onClick={() => approveMutation.mutate({ id: r._id, status: 'approved' })}
            >
              Approve
            </Button>
            <Button
              ...
              color="error"
              variant="outlined"
              onClick={() => approveMutation.mutate({ id: r._id, status: 'rejected' })}
            >
              Reject
            </Button>
          </Box>
        ...
        <CardContent sx={{ p: 3 }}>
          ...
          {tab === 1 && (
            <DataTable
              ...
              rows={pendingLeaves}
              loading={isLoading}
              searchKeys={['applicant.firstName', 'applicant.lastName']}
              emptyMessage="No pending student leave requests"
            />
          )}
        </CardContent>
      </Card>
            ...
            <Grid item xs={12}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  ...
                  label="Leave Type"
                >
                  {['sick', 'casual', 'earned', 'medical', 'personal', 'other'].map((t) => (
