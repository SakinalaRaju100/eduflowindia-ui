...
export default function SADashboard() {
  ...
  const isIdTaken = Boolean(
    form.schoolUniqueId &&
    schools.some((s) => s.schoolUniqueId === form.schoolUniqueId && s._id !== editSchool?._id),
  );
  ...
  const addStory = () =>
    setForm((p) => ({
      ...p,
      successStories: [...(p.successStories || []), { name: '', text: '', color: '#1565C0' }],
    }));
  const removeStory = (i) =>
    setForm((p) => ({ ...p, successStories: p.successStories.filter((_, ii) => ii !== i) }));
  const setStory = (i, k, v) =>
    setForm((p) => {
      const s = [...(p.successStories || [])];
      s[i] = { ...s[i], [k]: v };
      return { ...p, successStories: s };
    });
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent>
          ...
          <Grid container spacing={2}>
            {[
              ['name', 'School Name', 12],
              ['schoolUniqueId', 'Unique School ID', 12],
              ...
              <Grid item xs={12} sm={xs} key={k}>
                <TextField
                  ...
                  FormHelperTextProps={{
                    sx: {
                      color:
                        k === 'schoolUniqueId' && form.schoolUniqueId && !isIdTaken && !editSchool
                          ? 'success.main'
                          : undefined,
                      fontWeight: k === 'schoolUniqueId' ? 600 : 400,
                    },
                  }}
                  helperText={
                    k === 'schoolUniqueId'
                      ? editSchool
                        ? 'School ID cannot be changed once created.'
                        : isIdTaken
                          ? '❌ This School ID is already taken.'
                          : form.schoolUniqueId
                            ? '✅ Available'
                            : 'Auto-generated from Name. Modify to make unique if needed.'
                      : ''
                  }
                />
              </Grid>
            ...
            <Grid item xs={12} sm={6}>
              <FormControl fullWidth size="small">
                ...
                <Select
                  ...
                  onChange={(e) => setF('affiliationBoard', e.target.value)}
                  label="Board"
                >
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <TextField
                ...
                multiline
                rows={3}
                label="School Motive / About"
                value={form.schoolMotive || ''}
                onChange={(e) => setF('schoolMotive', e.target.value)}
              />
            </Grid>
            <Grid item xs={12}>
              <TextField
                ...
                onChange={(e) => setF('keypoints', e.target.value)}
              />
            </Grid>
          </Grid>
          <Box sx={{ mt: 2.5 }}>
            ...
            >
              <Typography variant="subtitle2" fontWeight={700}>
                Success Stories ({form.successStories?.length || 0})
              </Typography>
              <Button size="small" startIcon={<Add />} onClick={addStory}>
                Add Story
              </Button>
            </Box>
            {form.successStories?.map((s, i) => (
              <Box
                key={i}
                sx={{ p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 2, mb: 1.5 }}
              >
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                  <Typography variant="caption" fontWeight={700}>
                    Story {i + 1}
                  </Typography>
                  <IconButton size="small" onClick={() => removeStory(i)}>
                    <Delete fontSize="small" />
                  </IconButton>
                </Box>
                <Grid container spacing={1.5}>
                  ...
                  <Grid item xs={12} sm={2}>
                    <TextField
                      ...
                      label="Color"
                      sx={{ p: 0 }}
                      value={s.color || '#1565C0'}
                      onChange={(e) => setStory(i, 'color', e.target.value)}
                    />
                  </Grid>
                  <Grid item xs={12} sm={6}>
                    <TextField
        ...
        <DialogActions sx={{ p: 2.5 }}>
          ...
          <Button
            ...
            disabled={mutation.isPending || isIdTaken}
          >
            {mutation.isPending ? (
