...
export default function PrincipalSalaries() {
  ...
  const { data: salData, isLoading: sLoading } = useQuery({
    queryKey: ['salaries'],
  ...
  const handleOpenEdit = (sal) => {
    ...
    setForm({
      ...
      status: sal.status,
      paymentMode: sal.paymentMode || 'bank_transfer',
    });
    setOpen(true);
  };
  ...
  const cols = [
    {
      ...
      render: (r) => (
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          <Avatar
            src={r.teacher?.photo}
            sx={{ width: 32, height: 32, bgcolor: 'success.main', fontSize: 13, fontWeight: 700 }}
          >
            {r.teacher?.firstName?.[0]}
            {r.teacher?.lastName?.[0]}
          </Avatar>
          <Box>
        ...
        <Box sx={{ display: 'flex', gap: 0.5 }}>
          ...
          <Tooltip title="Delete">
            <IconButton
              ...
              onClick={(e) => {
                e.stopPropagation();
                if (window.confirm('Delete this salary record?')) delMutation.mutate(r._id);
              }}
            >
    ...
    <Box>
        ...
        <CardContent sx={{ p: 0 }}>
          <Box sx={{ p: 3 }}>
            <DataTable
              columns={cols}
              rows={salaries}
              loading={sLoading || tLoading}
              searchKeys={['teacher.firstName', 'teacher.lastName', 'month', 'year']}
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle fontWeight={700}>
          {editId ? 'Edit Salary Record' : 'Process New Salary'}
        </DialogTitle>
                ...
                <Select
                  ...
                  onChange={(e) => {
                    ...
                    setForm((p) => ({
                      ...p,
                      teacher: tId,
                      baseSalary: selectedT?.salaryDetails?.baseSalary || 0,
                      allowances: selectedT?.salaryDetails?.allowances || 0,
                      deductions: selectedT?.salaryDetails?.deductions || 0,
                    }));
                  }}
        ...
        <DialogActions sx={{ p: 2.5 }}>
          ...
          <Button
            ...
            disabled={mutation.isPending || !form.teacher || !form.baseSalary}
          >
            {mutation.isPending ? <CircularProgress size={20} color="inherit" /> : 'Save Salary'}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
