...
export default function PrincipalCalendar() {
  ...
  const { data } = useQuery({
    queryKey: ['calendar', format(start, 'yyyy-MM'), format(end, 'yyyy-MM')],
  ...
  const handleGenerateWeekends = async () => {
    ...
    try {
      const weekends = days.filter(
        (d) => getDay(d) == 0 && d.getMonth() === currentMonth.getMonth(),
      );
      const toCreate = weekends.filter((w) => {
        return !events.some(
          (e) =>
            (e.type === 'weekend' || e.title === 'Weekend') && isSameDay(new Date(e.startDate), w),
        );
      });
    ...
    <Box>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Button variant="outlined" onClick={handleGenerateWeekends} disabled={isGenerating}>
            {isGenerating ? (
              ...
              'Make Sundays as weekends for this month'
            )}
          </Button>
          <Button
            variant="contained"
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 2, px: 1 }}>
            {Object.entries(EVENT_COLORS).map(([type, color]) => (
              <Box key={type} sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                ...
                <Typography
                  variant="caption"
                  sx={{ textTransform: 'capitalize', fontWeight: 600, color: 'text.secondary' }}
                >
                  {type}
                </Typography>
              </Box>
                ...
                <Grid item xs={12 / 7} key={day.toISOString()} sx={{ minHeight: 80, p: 0.3 }}>
                  ...
                  >
                    <Typography
                      ...
                      sx={{
                        color: isToday ? '#fff' : isWknd ? 'error.main' : 'text.primary',
                        display: 'block',
                        mb: 0.3,
                      }}
                    >
                    ...
                    {dayEvents.map((ev) => {
                      ...
                      return (
                        <Box
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            if (window.confirm(`Delete "${ev.title}"?`)) delMutation.mutate(ev._id);
                          }}
                        >
                          <Typography
                            sx={{
            ...
            {events.length === 0 ? (
              ...
              events.map((ev) => {
                ...
                return (
                  <Chip
                    key={ev._id}
                    label={`${format(new Date(ev.startDate), 'dd MMM')} — ${ev.title}`}
                    size="small"
                    sx={{
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setForm((p) => ({
                      ...p,
                      type: e.target.value,
                      color: EVENT_COLORS[e.target.value] || '#607D8B',
                    }))
                  }
                  label="Type"
                >
        ...
        <DialogActions sx={{ p: 2.5 }}>
          ...
          <Button
            variant="contained"
            onClick={() => mutation.mutate({ ...form, targetRoles: ['all'] })}
