...
import { Box, Grid, Card, CardContent, Typography, Chip, LinearProgress, Avatar, List, ListItem, ListItemAvatar, ListItemText, Divider } from '@mui/material';
...
export default function PrincipalDashboard() {
  const { data: schoolProfile } = useQuery({ queryKey:['my-school'], queryFn:()=>api.get('/principal/school-profile').then(res=>res.data.data) });
  const { data: reports } = useQuery({ queryKey:['p-reports'], queryFn:()=>principalAPI.getReports() });
  const { data: attSummary } = useQuery({ queryKey:['att-summary'], queryFn:()=>attendanceAPI.getSchoolSummary() });
  const { data: announcements } = useQuery({ queryKey:['announcements'], queryFn:()=>announcementAPI.getAll() });
  const { data: classrooms } = useQuery({ queryKey:['classrooms'], queryFn:()=>principalAPI.getClassrooms() });
  ...
  return (
    <Box>
      ...
      <Grid container spacing={2.5} sx={{ mb:3 }}>
        <Grid onClick={()=>navigate('/principal/classrooms')} item xs={6} sm={3}><StatCard title="Classrooms" value={filteredClassrooms.length} icon={<Class/>} color="#6A1B9A" loading={!classrooms} /></Grid>
        <Grid onClick={()=>navigate('/principal/teachers')} item xs={6} sm={3}><StatCard title="Total Teachers" value={r.totalTeachers} icon={<Person/>} color="#2E7D32" loading={!reports} /></Grid>
        <Grid onClick={()=>navigate('/principal/students')} item xs={6} sm={3}><StatCard title="Total Students" value={r.totalStudents} icon={<People/>} color="#1565C0" loading={!reports} /></Grid>
        <Grid item xs={6} sm={3}><StatCard title="Today Present" value={att.present||0} subtitle={`of ${att.total||0} total`} icon={<TrendingUp/>} color="#E65100" loading={!attSummary} /></Grid>
      </Grid>
      ...
      <Grid container spacing={2.5}>
            ...
            <CardContent sx={{ p:3 }}>
              ...
              {attData.length > 0 ? (
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    <Pie data={attData} cx="50%" cy="50%" innerRadius={60} outerRadius={90} dataKey="value" paddingAngle={3}>
              ...
              ) : <Box sx={{ display:'flex', justifyContent:'center', alignItems:'center', height:220 }}><Typography color="text.secondary">No attendance data today</Typography></Box>}
            </CardContent>
          </Card>
        </Grid>
        ...
        <Grid item xs={12} md={7}>
          <Card elevation={0} sx={{ border:'1px solid', borderColor:'divider', borderRadius:3, height:'100%' }}>
            <CardContent sx={{ p:3 }}>
              <Typography variant="h6" fontWeight={700} gutterBottom>Students per Class</Typography>
              <ResponsiveContainer width="100%" height={220}>
        ...
        <Grid item xs={12}>
          <Card elevation={0} sx={{ border:'1px solid', borderColor:'divider', borderRadius:3 }}>
            <CardContent sx={{ p:3 }}>
              <Typography variant="h6" fontWeight={700} gutterBottom>Recent Announcements</Typography>
              {filteredAnnouncements.length === 0 ? <Typography color="text.secondary">No announcements for the selected year</Typography> :
                    ...
                    <Box sx={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
                      <Box>
                        <Box sx={{ display:'flex', alignItems:'center', gap:1, mb:0.5 }}>
                          ...
                          <Chip label={a.priority} size="small" color={a.priority==='urgent'?'error':a.priority==='important'?'warning':'default'} />
                        </Box>
                        <Typography variant="caption" color="text.secondary">{a.content?.slice(0,120)}...</Typography>
                      </Box>
