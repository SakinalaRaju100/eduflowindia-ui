...
const EMPTY_FORM = {
  ...
  gender: 'male',
  bloodGroup: '',
  guardians: [{ relationship: 'father', name: '', phone: '', email: '' }],
};
...
export default function PrincipalStudents() {
    ...
    mutationFn: async (d) => {
      ...
      if (editId) {
        ...
        if (String(oldClassroomId) !== String(newClassroomId) && userId) {
          if (oldClassroomId) {
            await api.post(`/classrooms/${oldClassroomId}/students`, {
              studentIds: [userId],
              action: 'remove',
            });
          }
          if (newClassroomId) {
            await api.post(`/classrooms/${newClassroomId}/students`, {
              studentIds: [userId],
              action: 'add',
            });
          }
        ...
        if (d.classroomId && userId) {
          await api.post(`/classrooms/${d.classroomId}/students`, {
            studentIds: [userId],
            action: 'add',
          });
        }
  ...
  const toggleStudentMutation = useMutation({
    mutationFn: (s) =>
      principalAPI.updateStudent(s._id, { isActive: s.isActive === false ? true : false }),
    onSuccess: () => {
      qc.invalidateQueries(['p-students']);
      setPopup(null);
    },
  ...
  const resetPasswordMutation = useMutation({
    mutationFn: (s) =>
      api.put(`/principal/students/${s._id}/reset-password`, {
        password: '1234',
        isFirstLogin: true,
      }),
    onSuccess: () => {
  ...
  const cols = [
    ...
    },
    {
      key: 'gender',
      label: 'Gender',
      render: (r) => <Chip label={r.gender} size="small" variant="outlined" />,
    },
    ...
    // { key:'password', label:'Password', render: r => <Typography variant="caption" fontWeight={600} color={r.userId?.isFirstLogin ? 'primary' : 'text.secondary'}>{r.userId?.isFirstLogin ? 'Student@1234' : 'Changed by user'}</Typography> },
    {
      ...
      render: (r) => (
        <Button
          ...
          onClick={(e) => {
            e.stopPropagation();
            if (window.confirm('Reset password to 1234?')) resetPasswordMutation.mutate(r);
          }}
        >
          Reset to 1234
        </Button>
    ...
    <Box>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="md" fullWidth>
        <DialogTitle fontWeight={700}>
          {editId ? 'Edit Student Details' : 'Enroll New Student'}
        </DialogTitle>
        <DialogContent>
          {error && (
          ...
          <Grid container spacing={2} sx={{ mt: 0.5 }}>
            {[
              ...
              ['email', 'Email', 4],
              ['phone', 'Phone', 4],
              ['rollNumber', 'Roll Number', 4],
              ['dateOfBirth', 'Date of Birth', 4],
            ].map(([k, l, xs]) => (
              <Grid item xs={12} sm={xs} key={k}>
                <TextField
                  ...
                  size="small"
                  label={l}
                  type={k === 'dateOfBirth' ? 'date' : undefined}
                  InputLabelProps={k === 'dateOfBirth' ? { shrink: true } : undefined}
                  value={form[k] || ''}
            ...
            {[
              ...
              <Grid item xs={12} sm={xs} key={k}>
                <TextField
                  ...
                  size="small"
                  label={l}
                  value={form.guardians?.[0]?.[k] || ''}
                  onChange={(e) => {
                    const g = [...form.guardians];
                    g[0] = { ...g[0], [k]: e.target.value };
                    setForm((p) => ({ ...p, guardians: g }));
                  }}
                />
              </Grid>
            ))}
      ...
      <StudentPopup
        ...
        onToggleStatus={(profile) => {
          if (window.confirm('Change student active status?'))
            toggleStudentMutation.mutate(profile);
        }}
      />
    </Box>
  );
