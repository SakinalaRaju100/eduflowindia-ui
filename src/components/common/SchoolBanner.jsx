...
export default function SchoolBanner({ propSchool }) {
  ...
  const school = propSchool || schoolProfile || (user?.school && typeof user.school === 'object' ? user.school : null);
  ...
  const logoSrc = school.logo || `https://ui-avatars.com/api/?name=${encodeURIComponent(school.name)}&background=1565C0&color=fff&size=128&bold=true`;
  ...
  return (
    <Box sx={{ mb: 3 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: images.length > 0 ? 2 : 0 }}>
        <Avatar src={logoSrc} variant="rounded" sx={{ width: { xs: 48, sm: 64 }, height: { xs: 48, sm: 64 }, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider' }} />
        <Box>
          <Typography variant="h5" fontWeight={800} sx={{ fontSize: { xs: '1.25rem', sm: '1.5rem' } }}>{school.name}</Typography>
          {(school.address?.city || school.address?.state) && (
            <Typography variant="caption" color="text.secondary">
              {school.address.city}{school.address.city && school.address.state ? ', ' : ''}{school.address.state}
            </Typography>
          )}
        </Box>
      </Box>
