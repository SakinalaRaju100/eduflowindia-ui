...
import { Card, CardContent, Box, Typography, Avatar, Chip, AvatarGroup, useTheme } from '@mui/material';
      ...
      <CardContent sx={{ p: 2.5 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' }}>
            <Box sx={{
              width: 48, height: 48, borderRadius: 2.5, bgcolor: color,
              display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff',
            }}>
              <Typography variant="h6" fontWeight={800}>{classroom.grade}</Typography>
            </Box>
            {(classroom.academicStartDate || classroom.academicEndDate) && (
              <Chip
                ...
                sx={{ fontSize: 11, height: 24, bgcolor: `${color}15`, color, fontWeight: 600, border: `1px solid ${color}30` }}
              />
            )}
          </Box>
          <Chip label={`Section ${classroom.section}`} size="small" sx={{ fontWeight: 700, bgcolor: `${color}15`, color }} />
        </Box>

        <Typography variant="h6" fontWeight={700} gutterBottom>{classroom.name || `Grade ${classroom.grade} - ${classroom.section}`}</Typography>

        {classroom.classTeacher && (
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
            <Avatar src={classroom.classTeacher?.photo} sx={{ width: 22, height: 22, fontSize: 10, bgcolor: color }}>
        ...
        <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            <Typography variant="caption" color="text.secondary">{classroom.students?.length || 0} students</Typography>
          </Box>
          {classroom.roomNumber && (
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
              <Room sx={{ fontSize: 14, color: 'text.secondary' }} />
              <Typography variant="caption" color="text.secondary">Room {classroom.roomNumber}</Typography>
            </Box>
          )}
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            ...
            <Typography variant="caption" color="text.secondary">{classroom.subjects?.length || 0} subjects</Typography>
          </Box>
        </Box>
      </CardContent>
    </Card>
