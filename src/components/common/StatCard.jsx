...
export default function StatCard({ title, value, icon, color = '#1565C0', trend, trendLabel, loading, subtitle, onClick }) {
  if (loading) return (
    <Card elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 3, cursor: onClick ? 'pointer' : 'default' }}>
  ...
  return (
    <Card elevation={0} onClick={onClick} sx={{
      ...
      '&:hover': onClick ? { transform: 'translateY(-3px)', boxShadow: `0 8px 24px ${color}20` } : {},
    }}>
      <CardContent sx={{ p: 2.5 }}>
